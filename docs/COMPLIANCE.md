...
| CIS 8 Audit Log Management | ⚠️ Pino structured logs ✅; the append-only `admin_audit_log` table is now written for admin state changes (first written by the category slice — create / update / reorder / delete, 2026-06-15 — extended by the product slice — create / update / reorder / delete / restore, 2026-06-22 — the banner slice — create / update / reorder / delete, 2026-06-29 — the store-settings slice — one row per save, 2026-06-30 — and the account-management slice — discount set / clear + account delete, 2026-07-03, which additionally logs admin PII *reads* (`admin_customer_viewed`) beyond state changes per 2026 insider-risk guidance — extended by the dashboard slice's `admin_dashboard_viewed` read log, 2026-07-06 — the archive slice — `category.restore` (un-archive) + the manual-backup `backup.create`, 2026-07-07 — and the snapshot-restore slice — `catalog.restore` (a whole-snapshot replay over the live catalog), 2026-07-08; order transitions are audited in `order_status_history`). SIEM no — acceptable at this scale |
