...
| CIS 8 Audit Log Management | ⚠️ Pino structured logs ✅; the append-only `admin_audit_log` table is now written for admin state changes (first written by the category slice — create / update / reorder / delete, 2026-06-15 — and extended by the product slice — create / update / reorder / delete / restore, 2026-06-22; order transitions are audited in `order_status_history`). SIEM no — acceptable at this scale |
