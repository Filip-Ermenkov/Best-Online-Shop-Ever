...
import { ArrowLeft, Check, Trash2, Plus, X, ImagePlus, GripVertical } from "lucide-react";
...
export default function AdminEditProductPage({ params }: Props) {
  ...
  const [variants, setVariants] = useState<Variant[]>(
    (product.variants ?? []).map((v) => ({
      ...v,
      options: v.options.map((o) => ({ ...o, imageUrl: o.imageUrl ?? "" })),
    }))
  );
  ...
  function addVariant() {
    ...
    setVariants((v) => [...v, { id: `var-new-${Date.now()}`, name: newVariantName.trim(), options: [] }]);
    setNewVariantName("");
  }
  function removeVariant(i: number) { setVariants((v) => v.filter((_, idx) => idx !== i)); }
  function addOption(variantIdx: number) {
    const label = prompt("Опция:");
    if (!label?.trim()) return;
    setVariants((v) => v.map((variant, i) =>
      i === variantIdx
        ? { ...variant, options: [...variant.options, { id: `opt-${Date.now()}`, label: label.trim(), available: true, imageUrl: "" }] }
        : variant
    ));
  }
  function toggleOption(vi: number, oi: number) {
    setVariants((v) => v.map((variant, i) =>
    ...
    <div className="max-w-2xl">
      <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold">Редакция</h1>
        </div>
        <Button variant="destructive" size="sm" className="gap-1.5" onClick={() => router.push("/admin/products")}>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <section className="rounded-lg border border-border bg-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input id="code" value={form.code} onChange={(e) => setField("code", e.target.value)} required className="mt-1" />
            </div>
            <div>
        ...
        {/* Category — flat list with depth indentation */}
        <section className="rounded-lg border border-border bg-card p-5 space-y-4">
          <h2 className="font-semibold">Категория</h2>
          <div>
            <Label>Категория (на всяко ниво)</Label>
            <select
        ...
        <section className="rounded-lg border border-border bg-card p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
            ...
            <div className="flex items-center gap-2 sm:pt-6">
              <input type="checkbox" id="isNew" checked={form.isNew as boolean} onChange={(e) => setField("isNew", e.target.checked)} className="w-4 h-4 accent-primary" />
        ...
        <section className="rounded-lg border border-border bg-card p-5 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button type="button" variant="outline" size="sm" onClick={addImageSlot} className="gap-1">
              <ImagePlus className="w-4 h-4" /> Добави
            </Button>
          </div>
            ...
            {imageUrls.map((url, i) => (
              <div key={i} className="flex items-start gap-2">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  {/* Sanitizer-by-transformation: bind the canonicalized
                     URL string the sanitizer returns, not the raw user
                     input. Blocks javascript:/data:/vbscript: URL XSS
                     AND closes "DOM text reinterpreted as HTML" /
                     "Client-side XSS" alert classes across CodeQL /
                     Snyk / Semgrep — see sanitizeImageUrl() in
                     lib/utils.ts for the full rationale. */}
                  {(() => {
        ...
        {/* Variants with option images */}
        <section className="rounded-lg border border-border bg-card p-5 space-y-4">
          <h2 className="font-semibold">Варианти</h2>
          {variants.map((variant, vi) => (
            <div key={variant.id} className="border border-border rounded-md p-3 space-y-3">
              <div className="flex items-center justify-between">
                <span className="font-medium text-sm">{variant.name}</span>
                <button type="button" onClick={() => removeVariant(vi)} className="text-muted-foreground hover:text-destructive">
                  <X className="w-4 h-4" />
                </button>
              </div>
              <div className="space-y-2">
                {variant.options.map((opt, oi) => (
                  <div key={opt.id} className="flex items-center gap-2 flex-wrap">
                    <button
                      type="button"
                      onClick={() => toggleOption(vi, oi)}
                      className={cn(
                        "px-2.5 py-1 text-xs rounded-md border transition-colors",
                        opt.available ? "border-primary/60 bg-primary/5" : "border-border opacity-40 line-through"
                      )}
                    >
                      {opt.label}
                    </button>
                    <Input
                      type="url" placeholder="Снимка URL"
                      value={opt.imageUrl}
                    ...
                    {/* Same sanitizer-by-transformation pattern as the
                       main product image preview above. */}
                    {(() => {
              ...
              <button type="button" onClick={() => addOption(vi)} className="px-2.5 py-1 text-xs rounded-md border border-dashed border-border hover:border-primary/60 text-muted-foreground">
                + Добави опция
              </button>
            </div>
          ))}
          <div className="flex gap-2">
            <Input placeholder="Нов вариант (напр. Размер)" value={newVariantName} onChange={(e) => setNewVariantName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addVariant())} className="flex-1" />
            <Button type="button" variant="outline" onClick={addVariant}><Plus className="w-4 h-4" /></Button>
          </div>
        </section>
        ...
        <div className="flex gap-3">
          ...
          <Button type="submit" className="flex-1 gap-2" disabled={saved}>
            {saved ? <><Check className="w-4 h-4" /> Запазено!</> : "Запази промените"}
          </Button>
        </div>
      </form>
    </div>
